...
window.onload = function(){
    ...
    var moveTypes = {
        light:  {id: 0, name:"light"},
        normal: {id: 0, name:"normal"},
        heavy:  {id: 0, name:"heavy"},
    };
    ...
    var GameMap = Class.create({
        ...
        moveFune: function(fune, path, onEnd) {
            if (path.length > 0) {
                ...
                var self = this;
                fune.tl.moveTo(worldPosition.x, worldPosition.y, 10, enchant.Easing.QUAD_EASEINOUT).then(function(){
                ...
                for (var rangeJ = -this.activeFune.getMovement(); rangeJ <= this.activeFune.getMovement(); rangeJ++) {
                    ...
                    if (!this.outOfBorders(targetI, targetJ)) {
                        if (this.getManhattanDistance(this.activeFune.i, this.activeFune.j, targetI, targetJ) <= this.activeFune.getMovement()) {
                            var path = this.getPath(this.activeFune.i, this.activeFune.j, targetI, targetJ);
                            if (path.length <= this.activeFune.getMovement()) {
